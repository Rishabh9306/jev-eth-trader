...
class JevEngine:
    ...
    def fallback_evaluate(
        ...
        trend_score = 0.0
        t1h = ind.get("trend_alignment_1h", "NEUTRAL")
        t5m = ind.get("trend_alignment_5m", "NEUTRAL")
        if t1h == "BULLISH" and t5m == "BULLISH":
            trend_score += 0.45  # Confirmed multi-timeframe bullish trend
        elif t1h == "BEARISH" and t5m == "BEARISH":
            trend_score -= 0.45  # Confirmed multi-timeframe bearish breakdown
        elif t1h == "BULLISH" and t5m != "BEARISH":
            trend_score += 0.25
        elif t1h == "BEARISH" and t5m != "BULLISH":
            trend_score -= 0.25
        elif t1h == "BULLISH" and t5m == "BEARISH":
            trend_score += 0.02  # Pullback inside uptrend
        elif t1h == "BEARISH" and t5m == "BULLISH":
            trend_score -= 0.02  # Bear rally inside downtrend
